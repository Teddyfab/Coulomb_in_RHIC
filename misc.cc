...
//! we wish to boost from "a given frame" to some frame moving in the +z direction with velocity 0.4c.
//! In this case, we use  u = gamma*( 1, 0.4, 0.4, 0.4 ) \f$ where : gamma = 1/sqrt(1-0.4^2) = 1.091089451
//! and p = gamma*m*( 1, 0.4, 0.4, 0.4 )  

void Boost(FourVector u, FourVector p, double px, double py){
       // cout<< u[0]<<"\n";
        //cout<<"The thermal momentum happens to be"<<"\n";
	//int mu;
	//double gamma = 1.091089451;
	//double gm1 = gamma * m1;
	double ndotp,udotn,udotp, TP,v2;
	//double px=0, py=0;
...
//printf("  u[0]         u[1]        u[2]       u[3]         p[0]           p[1]            p[2]                 p[3]          TP           v2\n");
	for(int mu=0;mu<4;mu++){
		p[mu]=-((udotp+ndotp)/(1.0+udotn))*u[mu]+2*ndotp*u[mu]+p[mu];
	      //cout<< "p["<<mu<<"] =" << p[mu]<<"\n";
	}
	         
	TP = sqrt(p[1]*p[1] + p[2]*p[2] + p[3]*p[3]);
	v2 = (p[1] - p[2])/(p[1] + p[2]) ;
	 	 	
	printf("%f   %f   %f   %f   %f   %f   %f   %f   %f    %f\n", 
            u[0], u[1], u[2], u[3], p[0], p[1], p[2], p[3], TP, v2);        
}
